	...
	while (*chs != '\0')
	{
		if (*chs > 0xa0)				//汉字内码都是大于0xa0
		{
			...
			{	
				...
				if ((*chs == hz_index[i*2]) && (*(chs+1) == hz_index[i*2+1]))//用户chs输入匹配库中汉字
				{
					OLED_Set_Pos(x, y);//匹配成功
					for(j=0;j<16;j++)
...
void OLED_Init(void)
{
  ...
  Write_IIC_Command(0x00); //---set low column addressp 偏移地址
  Write_IIC_Command(0x10); //---set high column address
  Write_IIC_Command(0x40); //--set start line address
  Write_IIC_Command(0xB0); //--set page address
  Write_IIC_Command(0x81); // contract control 对比度设置指令
  Write_IIC_Command(0xFF); //--128 调节亮度
  Write_IIC_Command(0xA1); //set segment remap
  Write_IIC_Command(0xA6); //--normal / reverse
  Write_IIC_Command(0xA8); //--set multiplex ratio(1 to 64) 设置分辨率
  Write_IIC_Command(0x3F); //--1/32 duty   128*64：0x3f
  ...
  Write_IIC_Command(0xDA); //set com pin configuartion 设置COM硬件引脚配置，适应分辨率
